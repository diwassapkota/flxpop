...
public class FonepaySocketManager {
    ...
    public FonepaySocketManager(TransactionRepository txnRepo,
                                ...
                                @Value("${flexpop.gateways.fonepay.socket-enabled:true}") boolean enabled) {
    ...
    @Scheduled(
            fixedDelayString   = "${flexpop.gateways.fonepay.socket-sweep-ms:3000}",
            initialDelayString = "${flexpop.gateways.fonepay.socket-initial-delay-ms:3000}"
    )
    public void sweep() {
        if (!enabled) return;
